...
- (id)init
{
    if ((self = [super init]))
    {
        ...
        self.thread = [[NSThread alloc] initWithTarget:self selector:@selector(emulationLoopThread) object:nil];
...
- (void) emulationLoopThread {
    ...
    while (!shouldStop) {
        ...
        @synchronized (self) {
            if (isRunning) {
                BOOL isSkip = false;
                if (self.frameSkipInterval != 0) {
                    isSkip = frame % self.frameSkipInterval != 0;
                }
                [self executeFrameSkippingFrame:isSkip];
                ++frame;
            }
        }
        frameCount += 1;
